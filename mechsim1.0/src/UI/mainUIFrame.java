...
public class mainUIFrame extends JFrame {
 ...
 public SimulationTab getSimuTab(){
  ...
  public void StopVisualisationThread(){
	  ...
	  public void BeginSimPressed(File chosenFile){
		  if (!SimulationRunning){
			  ChosenFile=chosenFile;
			  tabbedPane.remove(tabbedPane.getComponent(1));
			  mainThread cycle;
			  cycle = new mainThread(chosenFile);
			  createSimVisual(cycle);
	 ...
	 public void restartPressed(){
		 if(SimulationRunning){
			 EndSimPressed();
			 BeginSimPressed(ChosenFile);
			 
		 }
	 }
	 void createSimVisual(mainThread cycle){
